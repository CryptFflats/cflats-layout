...
import { setIsWlBoxActive, setWlBoxId } from '../../../../core/store/slices/WlBoxSlices';
...
const WlBoxModal = () => {
	...
	return (
		<Dialog onClose={() => dispatch(setIsWlBoxActive(false))} open={isWlBoxActive}>
			Input id of Whitelist Box
			<br />
			<span className={`${styles.errorText} ${getDisplayStyles}`}>Id should be a number</span>
			<br />
			<input
			...
			/>
			<br />
			<button className={`${styles.wlBoxBtn} ${styles.wlMt}`} onClick={async (e: any) => handleClick(e)}>USE WL BOX</button>
			<button className={`${styles.wlBoxBtn} ${styles.withoutWl}`} onClick={async (e: any) => handleMintWithoutWlBox(e)}>MINT WITHOUT WL BOX</button>
		</Dialog>
	);
};
